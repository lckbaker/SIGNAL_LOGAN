...
server <- function(session, input, output) {
  ...
  observeEvent(input$goButton, {
    ...
    if('SHINY_SERVER_VERSION' %in% env_names){
      ...
      source("/srv/shiny-server/Rscripts/pathway_iteration_GO.R", local = TRUE)
    }else{
      source("~/SIGNAL/app/Rscripts/pathway_iteration.R", local = TRUE)
      source("~/SIGNAL/app/Rscripts/pathway_iteration_GO.R", local = TRUE)
    }
    ...
    ############ This section handles gathering the pathways
    print('Database Selection')
    
    # Pathway types
    pathway.type <<- input$pathway_alt
    
    if(grepl("KEGG:", pathway.type ) ) {
      path.set <- 'KEGG'
      #Get appropiate pathway document suffix
      if (pathway.type == 'KEGG: Biological Processes'){
        pathway.type <- 'BiologicalProcesses'
      } else if (pathway.type == 'KEGG: Disease Pathways'){
        pathway.type <- 'Disease'
      } else if (pathway.type == 'KEGG: All Pathways'){
        pathway.type <- 'All'
      } 
      ...
      if(organism == 'Human') {
        ...
        Sym2Entrez <- function(x) {
          humanGenes[humanGenes$GeneSymbol %in% x,'EntrezID']
        }
        
      } else if(organism == 'Mouse') {
        
        pathwayData <- as.list(org.Mm.egGO2ALLEGS)
        Entrez2Sym <- function(x) {
          mouseGenes[mouseGenes$EntrezID %in% x,'GeneSymbol']
        }
        Sym2Entrez <- function(x) {
          mouseGenes[mouseGenes$GeneSymbol %in% x,'EntrezID']
        }
      } else {
        #Emergency case defaults to crashing the program - with no survivors
        stop('No animal selected')
      }
      ...
      if (pathway.type == 'GO: Biological Processes'){
        ...
        #Subsetting out the BP name as well (only sub pathways)
        pathwayData <<- pathwayData[names(pathwayData) %in% names(GO_Subset)]
        #This is the list of all Entrez IDs relevant
        gene_univ <- unique(unlist(pathwayData$`GO:0008150`))
        
        #This is the list of all pathways in Symbols, will take a moment to load 
        print('Making Gene symbol copy')
        pathwayData_Sym <- lapply(pathwayData, Entrez2Sym) 
        print('Finished gene symbol copy')
        
      } else if (pathway.type == 'GO: Molecular Functions'){
        pathway.type <- 'MF'
        #"GO:0003674"
        GO_Subset = as.list(GOMFOFFSPRING)
        
        pathwayData <<- pathwayData[names(pathwayData) %in% names(GO_Subset)]
        #This is the list of all Entrez IDs relevant
        gene_univ <- unique(unlist(pathwayData$`GO:0003674`))
      ...
      } else if (pathway.type == 'GO: Cellular Components') {
        #Check if this way works? May be too small and have to be cut
        pathway.type <- 'CC'
        #"GO:0005575"
        GO_Subset = as.list(GOCCOFFSPRING)
        
        pathwayData <<- pathwayData[names(pathwayData) %in% names(GO_Subset)]
        #This is the list of all Entrez IDs relevant
        gene_univ <- unique(unlist(pathwayData$`GO:0005575`))
      ...
      } else if (pathway.type == 'GO: All Pathways'){
        pathway.type <- 'All'
        pathwayData <<- pathwayData
        
        #This is the list of all Entrez IDs relevant
        gene_univ <- unique(unlist(pathwayData))
      ...
      #If you want to add more databases, modify this section
    }
    ...
    #I think this line was incomplete - Logan
    siRNA.Score <- 
    ...
    while (counter) {
      ...
      #Seperate into different pathway analysis here - just easier to maintain honestly
      print('pathset')
      print(path.set)
      if(path.set == 'KEGG' ) {
        print('Enrich KEGG')
        siRNA.Score <- ComputeEnrichment_KEGG(pathwayData, Hits, nonHits, outPrefix, siRNA.Score, iteration)
      } else if(path.set == 'GO' ) {
        print('Enrich GO')
        siRNA.Score <- ComputeEnrichment_GO(pathwayData, pathwayData_Sym, gene_univ, 
                                            Hits, nonHits, outPrefix, siRNA.Score, iteration)
      } else {
        print('failure to assign')
      }
    ...
    if(grepl("GO", path.set ) ) {
      ...
      pathEnrich2 <- pathEnrich2 %>%
        ...
        mutate(
          ...
          Genes = as.integer(strsplit(BgRatio, "/")[[1]][1]),
          HitGeneNames = str_replace_all(HitGeneNames, "/", ", ")
        ) %>%
        ungroup() %>%
        filter(HitGenes > 0) %>%
    ...
    print('G2')
    print(head(FinalEnrichment.df) )
    print(lapply(FinalEnrichment.df, class))
    
    ########### Generate Enrichment Score
    FinalEnrichment.df$EnrichScore <- NA
  
    for (i in 1:length(FinalEnrichment.df$Pathway)) {
      #This line is failing, likely a class error, check what it is in Kegg version and see whats up
